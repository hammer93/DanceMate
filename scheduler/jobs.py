...
def engine_reprocess(settings: Settings) -> str:
    ...
    detail = (
        ...
        f"blocked {result['blocked_selected']} sel/"
        f"{result['blocked_reconciled']} rec/"
        f"{result['blocked_preserved_input_loss']} kept/"
        f"{result['blocked_preserved_reviewed']} rev/"
        f"{result['blocked_failed']} err "
        f"stale_event_removed={result['stale_event_removed']} "
