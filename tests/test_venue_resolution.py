...
def test_a_region_hint_only_selects_a_region_it_actually_matches(pg, seoul_id):
    ...
    세종 (Sejong) rather than 전남: v0.89.4's migration 038 registered 전남
    along with 전북/충남/강원 (real, currently-registered Communities name
    each of them) - 세종 is a recognised _REGION_BY_ADMIN prefix this
    project deliberately left unregistered (no real source evidence for it
    yet - see that migration's own comment), so it is now the genuinely-
    unregistered case this test means to check."""
    assert venue_resolution.suggested_region_id(pg, "서울") == seoul_id
    assert venue_resolution.suggested_region_id(pg, None) is None
    assert venue_resolution.suggested_region_id(pg, "세종") is None
