...
_TONE = {cd.VERIFIED_NEW: "ok", cd.VERIFIED_EXISTING: "ok", cd.POSSIBLE_DUPLICATE: "warn",
         ...
         cd.CONFIRMED: "ok", cd.INFERRED: "warn", cd.UNKNOWN_DATE: "muted", "new": "warn"}
...
def _filters(request: Request, genres, regions) -> dict[str, str]:
    allowed = {
        ...
        "region": {r["code"] for r in regions} | {cd.REGION_UNRESOLVED},
        "classification": set(cd.CLASSIFICATIONS), "state": set(cd.REVIEW_STATES),
        "queue": {q.upper() for q in cd.QUEUES},
    }
    out = {}
...
def _filter_form(filters: dict[str, str], genres, regions) -> str:
    ...
    region_codes = [r["code"] for r in regions if r["code"] != "KR"]
    region_labels = {r["code"]: r["name"] for r in regions}
    region_labels[cd.REGION_UNRESOLVED] = "지역 미확정"
    queue_labels = {q.upper(): label for q, label in cd.QUEUE_LABELS.items()}
    return f"""
<form method="get" action="{BASE}" class="formgrid" style="margin:12px 0">
  <div><label>검토 큐</label>{_select("queue", options([q.upper() for q in cd.QUEUES], queue_labels), filters["queue"])}</div>
  ...
  <div><label>지역</label>{_select("region", options([*region_codes, cd.REGION_UNRESOLVED], region_labels), filters["region"])}</div>
...
def _evidence_cell(item: dict[str, Any]) -> str:
    """Activity provenance + classification reasons, made visible instead of
    left for the operator to guess (Section 12-13): the date, how sure the
    engine is of it (CONFIRMED/INFERRED/UNKNOWN), where it came from, and the
    exact reasons that drove the classification."""
    when = item.get("activity_date")
    confidence = item.get("activity_date_confidence") or cd.UNKNOWN_DATE
    conf_label = {cd.CONFIRMED: "확인됨", cd.INFERRED: "추정", cd.UNKNOWN_DATE: "근거 없음"}[confidence]
    parts = [E(when.isoformat() if when else "-") + " " + _badge(confidence, conf_label)]
    src = item.get("activity_evidence_source")
    ev_link = public_link(item.get("activity_evidence_url")) if item.get("activity_evidence_url") else None
    if ev_link:
        parts.append(f'<div class="note clip"><a href="{E(ev_link)}" target="_blank" '
                     f'rel="noopener noreferrer">{E(item.get("activity_evidence_title") or ev_link)}</a>'
                     + (f" ({E(src)})" if src else "") + "</div>")
    reasons = item.get("reasons") or []
    if reasons:
        parts.append(f'<div class="note clip">{E(" / ".join(reasons))}</div>')
    return "".join(parts)


def _duplicate_cell(item: dict[str, Any], group: list[dict[str, Any]]) -> str:
    """Every other candidate in this duplicate group, always shown - a
    stronger candidate must never be left invisible behind a weaker one that
    merely has a lower item_id (Section 8-9). The first (best-ranked) member
    is marked as a hint only; nothing here picks or merges automatically."""
    if not group:
        return ""
    best_id = group[0]["item_id"]
    links = []
    for member in group:
        if member["item_id"] == item["item_id"]:
            continue
        tag = " <span class=note>(추천)</span>" if member["item_id"] == best_id else ""
        links.append(f'<a href="#candidate-{member["item_id"]}">#{member["item_id"]} '
                     f'{E(member["candidate_name"] or "-")}</a>{tag}')
    return f'<div class="note">중복 그룹 ({len(group)}):</div>' + "<br>".join(links)


def _candidate_rows(view: str, items: list[dict[str, Any]], comms, genre_names,
                    group_map: dict[int, list[dict[str, Any]]]) -> tuple[list, list]:
    rows, attrs = [], []
    ...
    for item in items:
        ...
        duplicate_cell = _duplicate_cell(item, group)
        review_cell = E(cd.REVIEW_LABELS[item["review_state"]])
        if cd.has_new_evidence(item):
            review_cell += " " + _badge("new", "새 근거 발견")
        actions = []
        if item["review_state"] not in cd.DONE_STATES:
            ...
            if group and not item.get("duplicate_cleared"):
                actions.append(_action_form(view, iid, "separate", "서로 다른 Community"))
            actions.append(
                f'<form class="inline" method="post" action="{BASE}/items/{iid}/confirm-evidence">'
        ...
        rows.append([
            ...
            E(item["region_name"] or item["region_candidate"] or "지역 미확정"),
            ...
            ((existing if existing != "-" else "") + duplicate_cell) or "-",
            f'<div class="clip">{venues}</div>',
            review_cell,
...
def admin_discovery(request: Request, _: str = Depends(require_admin)) -> HTMLResponse:
    ...
    with _connection() as con:
        ...
        queue = filters["queue"].lower() or None
        total = cd.list_items(con, provider=filters["provider"] or None, genre=filters["genre"] or None,
                              ...
                              review_state=filters["state"] or None, queue=queue, limit=1)[1]
        ...
        items, total = cd.list_items(
            ...
            review_state=filters["state"] or None, queue=queue, limit=PAGE_SIZE,
    ...
    body = (
        ...
        "이루어집니다. 신뢰도와 분류는 판단을 돕는 참고 정보입니다. 최근 활동일은 실제 게시물 본문에 적힌 "
        "날짜를 우선하며, 검색 결과 메타데이터만으로 찾은 날짜는 '추정'으로 표시되고 자동으로 '확인됨'이 "
        "되지 않습니다.</p>"
        ...
        + admin._table(["이름 / 근거", "장르 후보", "지역", "Provider", "URL", "활동 근거", "신뢰도",
                        "분류", "기존 동호회 / 중복 그룹", "장소 후보", "검토", "처리"], rows,
...
def _register_page(item: dict[str, Any], values: dict[str, Any], return_to: str,
                   flash: tuple[str, str] | None, *, status_code: int = 200,
                   siblings: list[dict[str, Any]] | None = None,
                   checked_siblings: set[int] | None = None) -> HTMLResponse:
    with _connection() as con:
    ...
    evidence = admin._table(["항목", "값"], [
        ...
        ["활동일 / 근거 신뢰도", E(f'{item["activity_date"].isoformat() if item["activity_date"] else "-"} / '
                                f'{item.get("activity_date_confidence") or cd.UNKNOWN_DATE}')],
    ...
    sibling_block = ""
    if siblings:
        checked = checked_siblings or set()
        chips = "".join(
            f'<label class="chip"><input type="checkbox" name="sibling_ids" value="{s["item_id"]}"'
            f'{" checked" if s["item_id"] in checked else ""}> #{s["item_id"]} '
            f'{E(s["candidate_name"] or "-")}</label>' for s in siblings)
        sibling_block = (
            '<div class="field"><label>같은 Community로 함께 연결할 중복 후보</label>'
            '<p class="note">체크한 후보는 새로 등록되는 이 동호회에 함께 연결됩니다(같은 Community). '
            f'체크하지 않은 후보는 그대로 남습니다.</p><div class="chipset">{chips}</div></div>')
    body = (
        ...
        + _community_fields(values, masters) + sibling_block
...
def admin_register_form(item_id: str, request: Request, _: str = Depends(require_admin)) -> HTMLResponse:
    ...
    with _connection() as con:
        ...
        group = cd.duplicate_group(con, iid) if item else []
    if item is None:
    ...
    siblings = [m for m in group if m["item_id"] != iid and m["review_state"] not in cd.DONE_STATES]
...
async def admin_register(item_id: str, request: Request, reviewer: str = Depends(require_admin)):
    ...
    try:
        sibling_ids = parse_ids(_list(form, "sibling_ids"), what="중복 후보")
        with _connection() as con:
            if sibling_ids:
                result = cd.register_item_and_link_group(con, iid, values, sibling_ids, reviewer=reviewer)
        ...
        return _register_page(item, values, return_to, ("bad", f"등록하지 못했습니다: {exc}"),
                              status_code=400, siblings=siblings, checked_siblings=set(sibling_ids))
    return _go(return_to, BASE,
               f"동호회 '{community['name']}' 등록됨 (Community #{community['community_id']})",
               anchor=f"candidate-{iid}")


@router.post(BASE + "/items/{item_id}/confirm-evidence")
async def admin_confirm_evidence(item_id: str, request: Request, reviewer: str = Depends(require_admin)):
    """"근거 확인" (Section 6/15/16): an operator states they actually
    checked the page and found this date - the only path that ever sets
    activity_date_confidence to CONFIRMED (cd.confirm_activity_evidence)."""
    iid = _path_id(item_id)
    form = await request.form()
    return_to = _text(form, "return_to")
    try:
        with _connection() as con:
            cd.confirm_activity_evidence(
                con, iid, activity_date=_text(form, "activity_date"),
                evidence_url=_text(form, "evidence_url") or None,
                evidence_title=_text(form, "evidence_title") or None, reviewer=reviewer)
    except DirectoryError as exc:
